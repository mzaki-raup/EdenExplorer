...
- [x] **Tab close menu** - right-click a tab for Close, Close Other Tabs, Close Tabs to the Left, and Close Tabs to the Right (pinned tabs are always kept)
...
- [x] **Context Menu Order** - a dedicated Settings page to rearrange the sections of the file/folder right-click menu (Add To Favorites, Add/Remove Tag, Open In Default Program, Send To, Compress (Zip), Custom Context Menu Commands, Copy/Copy Path/Cut/Paste, Rename, Delete/Delete Permanently, Create Shortcut, Checksums, Properties, Windows Menu Item) in any order, add or remove separator lines anywhere between them, and reset back to the default order with one click
- [x] **Consistent Title Case context menu labels** - every right-click entry and tab tooltip now uses Title Case, with keyboard shortcuts shown in a uniform style (e.g. `Copy (Ctrl+C)`, `Copy Path (Ctrl+Shift+C)`, `Delete (Del)`)
- [x] **Richer tab right-click menu** - besides the close actions, a tab's own right-click menu offers Pin/Unpin Tab, Add To/Remove From Favorites, Save Search (on a search-results tab), opening or replacing your tabs with any saved Tab Group, and adding the tab (including both panes of a split tab) to a new or existing Tab Group
- [x] **Tab Group icons** - each Tab Group can follow its first folder's real shell icon (the default), use any glyph from the searchable icon picker, or use a custom image file; the icon shows in the "+" button's group menu, the tab right-click menu, and the Tab Groups settings list
- [x] **Tab Groups and Send To export/import** - back up or share just your Tab Groups or just your Send To destinations as their own JSON file, separate from a full settings export (which already includes them)
- [x] **Tag Icon Style** - choose Filled or Outline tag icons in Settings > Tags, applied consistently to the sidebar, tag-view tabs, the address bar, and the Tags settings page
- [x] **Tab Corner Radius** - an Appearance > Layout control that rounds the top corners of both active and inactive tabs, with an instant Live Preview reflection
